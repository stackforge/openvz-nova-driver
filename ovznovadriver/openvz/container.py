...
class OvzContainers(object):
    ...
    def list_vz_artifacts(cls):
        ...
        ctids = itertools.chain(
            # Directories in the searched directories are named with the CTID
            # of the container they represent. Generate a list of all CTIDs in
            # use by looking in these directories. This accounts for containers
            # that are deleted but still leave behind certain directories.
            (int(f)
            for dir_pth in directories_to_check
            for f in os.listdir(dir_pth)
            if os.path.isdir(os.path.join(dir_pth, f)) and is_int(f)),
            # Each container also comes with a series of config files. Each
            # file is named in the pattern <CTID>.<CONFIG>. Generate a list of
            # CTIDs based on the config files that are present. This accounts
            # for containers that were deleted but left behind their config
            # files.
            (int(f.split('.')[0])
            for f in os.listdir(CONF.ovz_ve_conf_dir)
